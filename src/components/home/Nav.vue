<template>
  <div class="tab-wrapper">
    ...
    <div>
      <ul class="tab-list"
          style="width: 185px; transition-timing-function: cubic-bezier(0.165, 0.84, 0.44, 1); transition-duration: 300ms; transform: translate(0px, 0px) translateZ(0px);">
        <li class="tab-item" v-for="(item, index) in navList">
                <span class="tab-name" @click="switch_nav(index)">
                  <router-link :to="'/home/'+item.type"
                               :class="{'link-active': item.isActive}"> {{item.name}} </router-link>
                </span>
        </li>
      </ul>
    </div>
